...
# The objective of this script its to assist STD9-mitm tool to
...
ctrl_c() {
  echo ""
  echo "[STD9-mitm] Abort module execution .."
  sleep 2
exit
}
...
if [ "$external" = "YES" ]; then
  echo "[STD9-mitm] Loging TCP/UDP Events .."
  ...
  echo "[STD9-mitm] Loging Events in: 67/UDP(dst) .."
  echo "   * Interface : $interface"
  echo "   * Modem Ip  : $modem"
  echo "   * Hour/Date : $hour"
...
sleep 1.5
  ...
  if [ -e beep-warning.beep ]; then
    ...
    #
    if [ -e parse.bin ]; then
      echo "[STD9-mitm] Loging Events in: 67/UDP(dst) .." >> $rhost.log
    ...
    if [ -e triggertwo.bin ]; then
      echo "[STD9-mitm] Loging Events in: 67/UDP(dst) .." >> $secund.log
    ...
    if [ "$external" = "YES" ]; then
      echo "[STD9-mitm] Loging Events .." >> warn.log
