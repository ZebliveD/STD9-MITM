...
# This script will parse tcp header data collected by STD9-mitm under MITM attacks.
# Basic we use STD9-mitm.sh tool to poison target host under LAN to be abble to
...
do
  ...
  if [ -e logs/IG.log ]; then
    ...
    if [ $warn = "yes" ]; then
      ...
      # for later review ( ../STD9-mitm/logs/192.168.1.71-header_capture.log ).
      #
      if [ -d logs ]; then
        ...
        echo "[ERROR] ../STD9-mitm/logs/$rhost-header_capture.log  [ NOT BUILD ]"
      fi
